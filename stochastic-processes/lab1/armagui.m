...
%
% If the button "KrÃd?v stabilitet" is marked, poles and zeros
% are not allowed outside the unit circle.
%
% The button "Simulera" gives a realization of the process.
%
...
%                         2003-01-24 A Change in function ensurestability by FN
...
if (isempty(data.poles) & isempty(data.zeros))
  ...
  if q==0
    txt=[txt ' for an AR(' int2str(p) ')-process'];
  else
    if p==0
      txt=[txt ' for an MA(' int2str(q) ')-process'];
    else
      txt=[txt ' for an ARMA(' int2str(p) ',' int2str(q) ')-process'];
    end
  end
end
...
for k=1:length(data.zeros)
  ...
  if data.stabilityflag
    ...
    if d>1
      ch=1;
%      pos=pos/d*(1-1e-7);   % Changed by FN 2003-01-24
      pos=pos/abs(pos)^2;    % Changed by FN 2003-01-24
    end
  else
    d=max(abs([real(pos) imag(pos)]));
    if d>1.2
      ch=1;
%      pos=pos/d*1.2;        % Changed by FN 2003-01-24
      pos=pos/abs(pos)^2;    % Changed by FN 2003-01-24
    end
  end
  data.zeros(k)=pos;
