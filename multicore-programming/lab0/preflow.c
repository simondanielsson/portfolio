...
struct list_t { // adjcency list type (linked list)
	edge_t*		edge;
	list_t*		next;
};
...
static void push(graph_t* g, node_t* u, node_t* v, edge_t* e)
{
	...
	if (u == e->u) {
		d = MIN(u->e, e->c - e->f);
		e->f += d;   // increase flow by e, or set it to the capacity
	} else {
		d = MIN(u->e, e->c + e->f);
		e->f -= d; // decrease flow by e, or set it to -capacity (since other direction), i.e. push as much as the edge allows
	}
...
int preflow(graph_t* g)
{
	...
	while ((u = leave_excess(g)) != NULL) {
		...
		while (p != NULL) {
			...
			// determine direction (u,v vs the edge's u,v config)
			if (u == e->u) {
				v = e->v;
				b = 1;
			} else {
			...
			// just push once if we are allowed to
